...
st.markdown("""
This third section is dedicated to the effects that the weather conditions can cause to the health conditions of the bees. The Dataset :green-badge[predicting-honeybee-health-from-hive-and-weather], of which you can learn more in the EDA section, is used to evaluate the possible influence of this factor. The inspections' result provided by the dataset indicates only the istant of the inspection, the weather recorded and the requirements met. In light of this, the first we can do is try to understand how often a failure is recorded because of certain parameters and what eventually is the parameters that fails the most. Here follows a representation of this:
""")
st.plotly_chart(bar_failures)
st.markdown("""Apparently, considered the bar plot, we could affirm the STRESSORS parameter is the one that mainly causes the inspections to fail. However, this affirmation is kind of **weak**, having the dataset based on just two countries and with a small number of observations. Indeed, this outcome can be better understood if we examine the mean result from the inspections, for each state:
""")
st.dataframe(inspections_mean_result_df)
st.markdown("""
In fact, the bar plot shows a high rate of failure connected to the STRESSORS factor because of the low success rate during inspections in North Carolina.
\nHowever, even though such information may not help us in our research, there are few interesting affirmations we can confirm by looking at the data. First of all, it might be useful to visualize the possible relations between weather data and inspection parameters. Here follows a heatmap that represents such information:
""")
st.plotly_chart(heatmap)
st.markdown("""
In light of these results, the fact the North Carolina shows a lower success rate for the inspections, in terms of weather effects, can be somehow explained if we think about the geography of the two states.\n
-   In Utah, there is a drier environment and higher ventilations, with stronger and fresher winds due to the slightly lower teperatures and the presence of more distributed mountain chains. Combined with the scarse precipitations, the state has less humidity.
...
\nNevertheless, what we have now grants us a good premise for what will be discussed in the section, where a :orange-badge[Gap Analysis] of the period :orange-badge[2009-2010] is carried out.
""")
