...
st.header("Analysis")
st.markdown("""
First, we want to see how much the honey production has changed over the years in US. To this end, the dataset :yellow-badge[US_honey_production.csv] is used to show, over the period 1995-2021, this trend.
...
st.markdown(
"""
Now we need a way to represent all this information. The most basic one is indeed to take the amount of honey produced by all states in the same year, compute the mean (for each year) and show the results in a scatter plot. However, it would be nice to take advantage of it and try to predict the trend up to our days.\n
Here follows a graphical representation of what was just said, using the linear regression method for prediction (the data chosen for training/testing are highlighted differently):  
"""
)
st.plotly_chart(production_prediction, width="stretch")
st.markdown(
"""
Clearly, if we project our view to 2026 (the year during which the code is being written), we cannot affirm this is a valid estimate because of the jump between 2009 and 2010, but instead of discarding this data, we can examine the production along with its value (price).
"""
)
...
-   from 2010: the 2 quantities coincide, meaning the quantity of honey produced by hives is all given to enterprises and directly sold on the market.\n
To what happened in the gap 2009-2010 is dedicated a specific analysis has been conducted (see Section "**Jump Analysis**").
\nWhat we do now is consider these information and, in light of them, the trend of the global cost (defined as value of production) of honey. This value is computed as the mean price per pound times the produced quantity. Note that the dataset does not speak about the influence of inflation, yet we expect the value to get higher and higher. Here follows what just explained, showing the total mean price per year:
"""
)
...
st.markdown(
"""
We don't know the actual quantity of honey that has been produced in these days, yet we are aware of the fact that a pound of honey costs from 8 and 15 dollars, considering only honey produced inside the US territory (not imported). The chart shows an estimate of approximately **10.290k** dollars, meaning the assumed quantity of honey produced shall vary **between 1.286k and 686k** (just divide the total for 2026 with the cost for a pound).
